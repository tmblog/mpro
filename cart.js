...
const Cart = {
    ...
    update(index, quantity) {
        ...
        if (quantity > 10) {
            this.showNotification('Maximum 10 items per product', 'warning');
            quantity = 10;
        }
    ...
    /**
     * Format price (gets currency symbol from page or defaults to Â£)
     */
    formatPrice(amount) {
        ...
        cart.forEach((item, index) => {
            ...
            const quantityControls = isFree ? 
                ...
                `<div class="btn-group btn-group-sm">
                    <button class="btn btn-outline-secondary" onclick="Cart.update(${index}, ${item.quantity - 1})">-</button>
                    <input type="number" class="form-control form-control-sm text-center" style="width: 3.75rem;" 
                           value="${item.quantity}" min="1" max="10" 
                           onchange="Cart.update(${index}, this.value)" />
                    <button class="btn btn-outline-secondary" onclick="Cart.update(${index}, ${item.quantity + 1})">+</button>
                </div>`;
            ...
            html += `
                <div class="cart-item mb-3 pb-3 border-bottom${freeClass}" data-cart-index="${index}">
                    <div class="d-flex justify-content-between align-items-start">
                        ...
                        ${item.is_free && item.linked_to ? '' : `<button class="btn btn-sm btn-link text-danger" onclick="Cart.remove(${index})" title="Remove"><i class="bi bi-trash"></i></button>`}
                    </div>
            ...
            switch (offer.type) {
                case 'free_on_spend':
                    ...
                    if (subtotal < offer.minSpend) {
                        ...
                        const chooseText = maxQty > 1 ? `Choose ${remaining} more ${itemText}` : `Choose your free item`;
                        messages.push({
                            type: 'unlocked',
                            icon: 'gift-fill',
                            text: `${offer.name} unlocked! ${chooseText}.`,
                            offer_id: offer.id
                        });
                    }
                    break;
