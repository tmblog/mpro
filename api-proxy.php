...
$result = match(true) {
    ...
    // POST /products/reorder/options - Reorder option groups on a product
    $method === 'POST' && $api_path === '/products/reorder/options'
        => api_post($shop, '/products/reorder/options', [
            'product_id' => $body['product_id'] ?? 0,
            'order' => $body['order'] ?? []
        ]),

    // POST /products/bulk/upsert - Bulk create/update products
    $method === 'POST' && $api_path === '/products/bulk/upsert'
        => api_post($shop, '/products/bulk/upsert', [
            'category' => $body['category'] ?? '',
