...
function process_checkout($data) {
    ...
    // Detect table mode
    $table_mode = !empty($data['table_mode']);
    $table_number = $table_mode ? (int)($data['table_number'] ?? 0) : null;
    ...
    // 3. Validate cart from server side (including availability + minimum order)
    $is_table_mode = !empty($data['table_mode']) || !empty($data['table_number']);
    $cart_validation = validate_cart_server_side($data['cart'] ?? '[]', $data['delivery_method'] ?? null, $is_table_mode);
    if (!$cart_validation['valid']) {
        return json_response([
            'success' => false,
            'error' => $cart_validation['message'],
            'error_type' => $cart_validation['error_type'] ?? 'cart_error',
            'unavailable' => $cart_validation['unavailable'] ?? [],
            'min_order' => $cart_validation['min_order'] ?? null,
            'shortfall' => $cart_validation['shortfall'] ?? null
        ], 400);
    }
    
    // 4. Re-validate postcode if delivery (skip for table mode)
    $delivery_data = null;
    $uber_quote_data = null;
    if (!$table_mode && $data['delivery_method'] === 'delivery') {
        ...
        // 4a. Uber Direct - validate/refresh quote if enabled
        require_once CORE_PATH . 'functions/uber.php';
        if (is_uber_enabled()) {
            $uber_quote_id = $data['uber_quote_id'] ?? null;
            $uber_quote_expires = $data['uber_quote_expires'] ?? null;

            if (empty($uber_quote_id) || empty($uber_quote_expires)) {
                return json_response([
                    'success' => false,
                    'error' => 'Uber delivery quote is required. Please try again.',
                    'error_type' => 'uber_quote_missing'
                ], 400);
            }

            // Build dropoff data for quote validation
            $dropoff = [
            ...
            $quote_validation = validate_or_refresh_quote(
                $uber_quote_id,
                $uber_quote_expires,
                $dropoff
            );

            if (!$quote_validation['valid']) {
                return json_response([
                    'success' => false,
                    'error' => $quote_validation['error'] ?? 'Uber delivery is not available for this address. Please try again.',
                    'error_type' => 'uber_quote_invalid'
                ], 400);
            }

            // Store validated/refreshed quote data
            $uber_quote_data = [
                'quote_id' => $quote_validation['quote_id'],
                'lat' => $dropoff['lat'],
                'lng' => $dropoff['lng'],
                'refreshed' => $quote_validation['refreshed']
            ];
        }
    }

    // 4b. Validate offer/free items
    $offer_validation = validate_cart_offers($cart_validation['cart']);
    if (!$offer_validation['valid']) {
        return json_response([
            'success' => false,
            'error' => implode(', ', $offer_validation['errors']),
    ...
    $continue_as_guest = !empty($data['continue_as_guest']);
    if ($existing_user && !is_logged_in() && !$continue_as_guest) {
        return json_response([
            'success' => false,
            'error' => 'account_exists',
            'email' => $data['email'],
            'message' => 'We found an account with this email. Please login to continue.',
            'benefits' => get_login_benefits($cart_validation['totals']['subtotal'])
        ], 409);
    }
    ...
    // 6b. Validate and apply loyalty points redemption
    $points_to_redeem = 0;
    $points_discount = 0;

    if (!empty($data['points_to_redeem']) && is_logged_in()) {
        $points_to_redeem = (int)$data['points_to_redeem'];
        
        if ($points_to_redeem > 0) {
            require_once CORE_PATH . 'functions/loyalty.php';
            
            // Calculate remaining subtotal after promo discounts
            $remaining_subtotal = $totals['subtotal'] - $totals['discount'];
            
            // Validate the redemption
            $points_validation = validate_points_redemption(
                $_SESSION['user_id'],
                $points_to_redeem,
                $remaining_subtotal,
                $promo_codes
            ...
            if (!$points_validation['valid']) {
                return json_response([
                    'success' => false,
                    'error' => $points_validation['error']
                ], 400);
            }
            
            $points_discount = $points_validation['discount'];
            $totals['points_discount'] = $points_discount;
            $totals['final'] = max(0, $totals['final'] - $points_discount);
        }
    }

    $totals['points_to_redeem'] = $points_to_redeem;
    
    // 7. Create customer record (guest or registered)
    $customer_id = create_or_get_customer($data, $existing_user);

    // 8. Create order
    $order_result = create_order($data, $customer_id, $totals, $delivery_data, $uber_quote_data);
    ...
    if ($data['payment_method'] === 'cash') {
        ...
        // Update customer totals (always, regardless of loyalty)
        if (is_logged_in()) {
            update_customer_order_totals($_SESSION['user_id'], $totals['final']);
        }

        // Award/deduct loyalty points for cash orders (card orders handled in complete_ryft_payment)
        if (is_logged_in() && !empty($GLOBALS['site_config']['features']['loyalty'])) {
            require_once CORE_PATH . 'functions/loyalty.php';
            process_loyalty_on_acceptance($order_id);
        }
    }
...
function validate_checkout_form($data) {
    ...
    // Delivery method
    if (empty($data['delivery_method']) || !in_array($data['delivery_method'], ['delivery', 'collection', 'table'])) {
    ...
    // Time selection (not required for table orders)
    if (empty($data['table_mode']) && empty($data['order_time_type'])) {
        $errors['order_time'] = 'Please select ASAP or scheduled time';
    }

    // Scheduled time validation (skip for table mode)
    if (empty($data['table_mode']) && $data['order_time_type'] === 'scheduled') {
        if (empty($data['scheduled_time'])) {
            $errors['scheduled_time'] = 'Please select a time slot';
        } else {
            // Validate scheduled time is still available
            $scheduled_timestamp = strtotime($data['scheduled_time']);
            $ready_minutes = get_ready_minutes($data['delivery_method'] ?? 'collection');
            $minimum_timestamp = time() + ($ready_minutes * 60);
            
            if ($scheduled_timestamp < time()) {
                $errors['scheduled_time'] = 'Selected time has passed. Please choose a later time.';
            } elseif ($scheduled_timestamp < $minimum_timestamp) {
                $errors['scheduled_time'] = "Selected time is too soon. Please allow at least {$ready_minutes} minutes.";
            }
        }
    }
    ...
    // Terms acceptance (not required for table orders)
    if (empty($data['table_mode']) && empty($data['terms_accepted'])) {
        $errors['terms'] = 'Please accept terms and conditions';
    }
    
    // Determine error type for frontend handling
    $error_type = null;
    if (!empty($errors['scheduled_time'])) {
        $error_type = 'scheduled_time_invalid';
    }
 ...
 * Validates: existence, stock, availability, prices, quantities, minimum order
 */
function validate_cart_server_side($cart_json, $delivery_method = null, $is_table_mode = false) {
    ...
    foreach ($cart as $item) {
        ...
        // Use server-side product name
        $product_name = $product['name'];
        if (isset($product['parent_name'])) {
            $product_name = $product['parent_name'] . ' - ' . $product_name;
        }
        ...
        // Sanitize quantity (1-99)
        $quantity = max(1, min(99, (int)($item['quantity'] ?? 1)));
        
        // Validate and convert options
        $product_group_ids = array_column($product['options'] ?? [], 'group_id');
        $converted_options = [];
        $validated_options = [];
        
        foreach ($item['options'] ?? [] as $opt) {
            if (!isset($opt['group_id'], $opt['option_id'])) continue;
            
            $group_id = (int)$opt['group_id'];
            $option_id = (int)$opt['option_id'];
            
            // Skip if group not allowed for this product
            if (!in_array($group_id, $product_group_ids)) continue;
            ...
            $validated_options[] = [
                ...
                'option_price' => 0, // Will be recalculated
        ...
        $server_price = calculate_product_price($product_id, $converted_options, $is_table_mode);

        $claimed_free = !empty($item['is_free']);
        $line_total = $claimed_free ? 0 : round($server_price * $quantity, 2);

        $validated_cart[] = [
            'product_id' => $product_id,
            'product_name' => $product_name,
            'base_price' => $is_table_mode && isset($product['table_price']) 
            ? (float)$product['table_price'] 
            : (float)$product['price'],
            'options' => $validated_options,
            'quantity' => $quantity,
            'total_price' => $claimed_free ? 0 : $server_price,
            'line_total' => $line_total,
            'original_price' => $server_price,
            'cpn' => $claimed_free ? 0 : (int)($product['cpn'] ?? 1),
            'is_free' => $claimed_free,
    ...
    // Unavailable items error
    if (!empty($unavailable_items)) {
        $names = array_column($unavailable_items, 'product_name');
        return [
            'valid' => false,
            'error_type' => 'unavailable_items',
            'message' => 'Some items are no longer available: ' . implode(', ', $names),
    ...
    if ($delivery_method) {
        ...
        if ($min_order > 0 && $subtotal < $min_order) {
            return [
                ...
                'message' => sprintf(
                    'Minimum order for %s is £%.2f. Your cart total is £%.2f.',
                    $delivery_method,
                    $min_order,
                    $subtotal
                ),
 ...
 */
function get_login_benefits($subtotal) {
    ...
    if (!empty($site_config['features']['loyalty'])) {
        ...
        $points_earned = ($subtotal * $loyalty_config['earnRate']);
...
function validate_promo($promo, $subtotal, $customer_id, $email = null, $delivery_method = null) {
    ...
    if ($subtotal < $promo['minOrder']) {
        return ['valid' => false, 'message' => 'Minimum order £' . $promo['minOrder'] . ' required'];
    }
 ...
 * Create order record
 */
function create_order($data, $customer_id, $totals, $delivery_data, $uber_quote_data = null) {
    $db = get_db();
    // Detect table mode and set defaults
    $table_mode = !empty($data['table_mode']);
    if ($table_mode) {
        $data['order_time_type'] = 'asap';
        $data['delivery_method'] = $data['delivery_method'] ?? 'table';
    }
    ...
    // Get points to redeem (stored as intent, deducted on acceptance)
    $points_to_redeem = $totals['points_to_redeem'] ?? 0;
    $points_discount = $totals['points_discount'] ?? 0;

    // Add points discount to total discount for display/receipt purposes
    $total_discount = $totals['discount'] + $points_discount;
    ...
    $stmt = mysqli_prepare($db,
        "INSERT INTO orders (
            ...
            loyalty_points_redeemed, table_number
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    );

    $table_number = !empty($data['table_number']) ? (int)$data['table_number'] : null;

    mysqli_stmt_bind_param($stmt, "isssssisdssddsddssssii",
        $customer_id,
        $customer_type,
        $data['email'],
        $order_time,
    ...
    // Store Uber quote data if present
    if ($uber_quote_data && !empty($uber_quote_data['quote_id'])) {
        $uber_update = [
            'uber_quote_id' => $uber_quote_data['quote_id']
        ];
        if (!empty($uber_quote_data['lat'])) {
            $uber_update['delivery_lat'] = (float) $uber_quote_data['lat'];
        }
        if (!empty($uber_quote_data['lng'])) {
            $uber_update['delivery_lng'] = (float) $uber_quote_data['lng'];
        }
        db_update('orders', $uber_update, ['order_id' => $order_id]);
    }
...
function create_order_products($order_id, $cart) {
    ...
    $receipt_order = 1;
    $cart_key_to_order_product_id = [];

    // First pass: insert non-free items
    foreach ($cart as $item) {
        if (!empty($item['is_free'])) continue;
        
        $order_product_id = insert_order_product($db, $order_id, $item, $receipt_order, 0, $item['offer_id'] ?? null, null);
        
        if (!empty($item['cart_key'])) {
            $cart_key_to_order_product_id[$item['cart_key']] = $order_product_id;
        }
        
        $receipt_order++;
    }
    
    // Second pass: insert free items with resolved linked_to
    foreach ($cart as $item) {
        if (empty($item['is_free'])) continue;
        
        $linked_to_id = null;
        if (!empty($item['linked_to'])) {
            $linked_to_id = $cart_key_to_order_product_id[$item['linked_to']] ?? null;
        }
        
        insert_order_product($db, $order_id, $item, $receipt_order, 1, $item['offer_id'] ?? null, $linked_to_id);
        $receipt_order++;
    }
}

/**
 * Insert single order product
 */
function insert_order_product($db, $order_id, $item, $receipt_order, $is_free = 0, $offer_id = null, $linked_to_id = null) {
    ...
    // Store structured JSON for KDS/reprinting
    $options_data = !empty($item['options']) ? json_encode($item['options']) : null;
    
    // Pre-formatted with proper indentation for KDS display
    $options_formatted = '';
    if (!empty($item['options'])) {
        $rendered = [];
    ...
    // Use line_total for paid items, 0 for free items
    $price_total = $is_free ? 0 : ($item['line_total'] ?? ($item['total_price'] * $item['quantity']));
    
    $stmt = mysqli_prepare($db,
        "INSERT INTO order_products 
        (order_id, product_id, product_name, product_name_only, options_data, options_formatted, 
         product_quantity, product_price_total, receipt_order, is_free, offer_id, linked_to_id) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    );
    
    mysqli_stmt_bind_param($stmt, "iissssidissi",
        $order_id,
        $item['product_id'],
        $product_name,
        $product_name_only,
 ...
 * Called on every completed order, regardless of loyalty being enabled
 */
function update_customer_order_totals($customer_id, $order_total) {
    $db = get_db();
    ...
    mysqli_stmt_bind_param($stmt, "di", $order_total, $customer_id);
    mysqli_stmt_execute($stmt);
}