...
const ProductModal = {
    ...
    renderAllergens() {
        ...
        const allergenIds = this.state.productData.allergens || [];
        
        if (allergenIds.length > 0 && window.ALLERGENS) {
            const allergenNames = allergenIds
                .map(id => window.ALLERGENS[id] || id)
                .filter(Boolean);
            
            if (allergenNames.length > 0) {
                allergensEl.innerHTML = '<span class="allergens-label">Contains: </span>' + 
                    allergenNames.map(name => 
                        `<span class="product-modal__allergen">${name}</span>`
                    ).join('');
    ...
    renderOptionButton(groupId, option, context, parentOptionId, groupType) {
        ...
        if (isSelected && context === 'main') {
            const revealedGroupIds = this.getRevealedByForOption(groupId, option.id);
    ...
    getRevealedByForOption(groupId, optionId) {
        const groupsToReveal = [];
        const compositeKey = `${groupId}:${optionId}`;
        
        this.state.productConfig.forEach(cfg => {
            if (cfg.revealed_by && Array.isArray(cfg.revealed_by)) {
                // New format: "group_id:option_id" strings
                if (cfg.revealed_by.includes(compositeKey)) {
    ...
    handleSingleSelect(groupId, optionId, context, parentOptionId) {
        ...
        if (context === 'main') {
            const revealedGroupIds = this.getRevealedByForOption(groupId, optionId);
    ...
    validate() {
        ...
        Object.keys(this.state.selections.main).forEach(groupId => {
            Object.keys(this.state.selections.main[groupId]).forEach(optionId => {
                const revealedGroupIds = this.getRevealedByForOption(parseInt(groupId), parseInt(optionId))
